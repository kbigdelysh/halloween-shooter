...
public class Cannon : MonoBehaviour
{
    ...
    public void FireCannon()
    {
        ...
        {
            var spawnedBall = ObjectPool.SharedInstance.GetPooledObject();
            if (spawnedBall != null)
            {    
                spawnedBall.transform.SetPositionAndRotation(_cannonballSpawnPoint.transform.position, _cannonTransform.rotation);
                spawnedBall.SetActive(true);
                spawnedBall.GetComponent<Cannonball>().InvokeDisappear();
                if (spawnedBall.GetComponent<Rigidbody>() == null)
                {
                    spawnedBall.AddComponent<Rigidbody>();
                }
